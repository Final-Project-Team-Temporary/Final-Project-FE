...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const backendResponse = await fetch(`${process.env.BACKEND_URL}/api/auth/kakao`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.redirect(new URL('/login?error=로그인 처리 중 오류가 발생했습니다', request.url))
  }
}